...
function getCongressData(n){
	...
	var sunlight_url = "https://congress.api.sunlightfoundation.com/votes?&congress=112&chamber=house&per_page=50&page="+
	...
	$.ajax({
		...
		success: function(data){
			...
			//A D3 axis that does not work in this situation because we want to show time spans, not distinguish what the vote's index is.
			//You can make an axis using a 'time' scale, but dates are a complicated issue in javascript and D3.
			//You can read more here: https://github.com/mbostock/d3/wiki/Time-Scales
			//and here: https://github.com/mbostock/d3/wiki/Time-Formatting
			// var yAxis = d3.svg.axis().scale(yScale).orient("left")
			// myYaxis = g_axes.append("g")
   //            .attr("class", "yaxis")
   //            .attr("transform", "translate("+(w*0.5)+",0)")
   //            .call(yAxis);
...
function updateTimeline(cy, voteTime){
	...
	startTimeLine.data([linedata]).transition().duration(555).attr('d', lineFunction);

	//Some (terrible) logic to restrict the amount of dates that show up (fallible because of the async queries and other reasons).
	if(previous_month+2 < parseInt(dateParts[1]) || previous_year < parseInt(dateParts[0])){
